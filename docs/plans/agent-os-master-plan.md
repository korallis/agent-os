...
**Phase 6 — Console completion (2 wk)** — first–fifth slices shipped (`tooling/gates/phase-6.mjs` **G1–G14**; CI Playwright chromium) plus provider wizard E2E (`phase-6-wizard.mjs` W1–W4) and Figma-fidelity (`phase-6-fidelity.mjs` F1–F4); open items below are soak/terminal only, not the §7 remainder frame set:
...
- [ ] Task Detail: embedded terminal section + a **10-minute** sustained no-dropped-frames soak. The seq mechanism and its gap detection now exist and are gated (`phase-6-terminal.mjs`, 12 contiguous frames observed); per-column cost/token meters already render on the fusion columns. What remains is the long soak itself, deliberately not claimed on the strength of a 6-second run. Session Detail read-only attach shipped in fourth slice above. [A]+[R2]+[R3]
