...
### 1.5 The marketing site's fate — DECIDED, then REVERSED

**[R9] Captain's ruling (2026-07-26): the marketing site is REMOVED.** *"what part of this is a local Web app do you not understand, there really isnt a need for a marketing page at all… these can either be removed or repurposed."* `apps/marketing` is deleted, along with every `packages/ui` export only it consumed (`SiteHeader`, `GlassCard`, `MagneticButton`, `MicroLabel`, the `TextAnimations` primitives, the motion variants and the bundled Geist fonts). `packages/ui` now exports `cn` and the theme stylesheet — the only two things `apps/console` ever imported from it.

*Why this reversal was right and the original decision was not:* Agent OS is a local-only, single-user web app. A public marketing site for it had no audience, and maintaining one forced the project to make claims it could not evidence — an entire remediation pass (fabricated customer testimonials, SOC 2 / GDPR / HIPAA presented as held certifications, a 99.9% uptime SLA for a service nobody runs, subscription tiers for something not for sale, and a contact form that took a visitor's work email and silently discarded it) was underway before the Captain stopped it. The honest fix for a page that must not claim anything is not a smaller set of claims; it is no page. **Everything below that assumed a marketing site should be read through this ruling.**

*Superseded text, retained so the reversal is legible:* **[A] chosen over [B].** Preserved **verbatim as `apps/marketing`**, a separate Next.js app sharing `packages/ui`; deploys publicly; never imports orchestrator code. *Rationale:* a localhost-only console must never share a deployable with a public site. *Adopted from [B]:* the honesty pass. *Rejected alternative [B]:* `/site` fold-in with redirects.

*Consequences of [R9] elsewhere in this plan:* every "marketing renders identically" parity gate is **void** (there is nothing to render); `packages/ui` is single-source from the Figma file rather than dual-source; the §7 inventory's Landing Page row (`8:11470`) and the "future/marketing candidates" note under the R6.3-Q1 skip list are **dead** — those frames map to no app. The `packages/ui` design-token home and the Figma-fidelity mandate for `apps/console` are **unaffected**.

**[R5→R6.2→R6.3→R9] Design-system mandate (Captain's directives, current form):** **[R6.3]** the UI's single source of truth for the **product app** is the Captain's Figma file — *"AgentOS — AI Agent Orchestration Dashboard"* (`Ria7UpyEPRd9jNlF9B6xgF`); every product screen must **exactly replicate** its corresponding Figma frame (§7). **[R9]** `packages/ui` is the shared token/component home, **single-sourced from that Figma file** (dark charcoal surfaces, slim icon rail, top bar, stat cards with delta chips, teal→green charts, status pills — tokens and `cn` in `theme.css` / package surface). The dual-source framing and marketing-component promotion list are **void**. The R6.2 rule against bespoke lookalikes stands: components come from `packages/ui`, built once against the Figma spec. Ad-hoc styling in `apps/console` is a review defect.
...
| Browser terminal attach | **Read-only WS** (`ws` on HTTP upgrade): mint `POST /v1/sessions/:id/attach-ticket`, redeem once on `WS /v1/pty?ticket=`; pane content polled via `tmux capture-pane` (send-on-change) with **monotonic per-stream `seq`** (gap = drop; reconnect renumbers from 1; `closed` carries `lastSeq`). **Not** node-pty; client keystrokes get an explicit read-only notice. Take-over = Captain runs the display-only attach command in their own terminal | **[Phase 6 fourth slice + continuity]** — deliberate trade vs node-pty / pipe-pane (no native addon, no write path); frame contract in §8.2 |
...
| 16 | **[R7]** Auto-balancer: enabled toggle, roster (≥ 2 families required at write time), `steerAwayPct` (below Brain handoff threshold), `useReportedCost` tiebreak | `balancer.json5` | disabled; empty roster | yes |
...
├── .github/workflows/
│   ├── ci.yml                        # typecheck/lint/build/test + phase-1…6/6-wizard/6-fidelity/6-terminal/7/8/9/10 gates
...
│   │       │   ├── pipeline/page.tsx           # [Phase 9] live no-mistakes gate view (transport-honest)
│   │       │   ├── runs/page.tsx               # Live Log Stream (SSE; fusion.* + prompt.installed + pipeline.*)
...
│   │       ├── components/{shell,fleet,pipeline,runs,sessions,policies,settings,tasks,projects,providers,analytics,network,alerts,notifications,onboarding}/
│   │       └── lib/{daemon.ts, useEventStream.ts, useTaskEvents.ts, fetchTaskEvents.ts, fetchEventsReplay.ts, selectPrimaryQuotaMetric.ts, useDebouncedRefreshKey.ts, pipelineLogState.ts, formatNetwork.ts}
│   └── orchestrator/                 # agentosd + agentos CLI (colocated)
│       ├── package.json              # bin: { agentos, agentosd }
│       ├── defaults/                 # [R3] shipped Policy Pack: *.json5 (incl. observability.json5 #11) + prompts/**
...
│           ├── pipeline/             # [Phase 9] read-only no-mistakes watcher (SQLite+WAL+log tails)
│           ├── observability/        # [Phase 9] visibility profile matching (surface / wakeOn)
│           ├── server/               # fastify: health, config, events SSE, fleet/tasks/projects/tools/fusion/prompts/analytics/attach-ticket/afk/pipeline
│           ├── pty/                  # [Phase 6] tickets + read-only capture-pane WS (/v1/pty; per-stream seq)
...
│   ├── ui/                           # [R9] single-source: Figma tokens + `cn` only
│   │                                 #   theme/styles — no marketing-only component exports
...
└── tooling/
    ├── gates/                        # phase-{1,2,2b,3,4,5,6,6-terminal,7,8,9,10}.mjs + phase-6-wizard / phase-6-fidelity + lib/ports.mjs (Chromium-safe pickPort)
    │                                 #   (2b: Phase 2 fixture completion; phase-6/8/9: Playwright + real daemon where noted; phase-6-terminal: PTY seq soak + reconnect; phase-7: secondmates; phase-9: pipeline visibility)
    ├── evidence/                       # figma-fidelity.json route→frame manifest; capture helpers
    │   ├── figma-fidelity.json
    │   ├── capture-console.mjs         # full Console proof packs (docs/qa/runs/)
    │   └── capture-pipeline.mjs        # [Phase 9] pipeline-live + schema-drift screenshots
    └── screenshots/capture-console.mjs  # Playwright PR evidence (pnpm screenshots)
```

**[R3] change notes:** `core/` → `substrate/` (decision logic removed; state-machine validation remains); the Brain **tool surface** lives under `fleet/tool-surface.ts` (not a separate `substrate/tool-surface`); shipped defaults live in `apps/orchestrator/defaults/` and are installed to `~/.agentos/config/` templates on init. **Colocation note:** CLI lives under `apps/orchestrator` (`agentos` + `agentosd` bins), not a separate `apps/cli`; event persistence is `packages/event-store` (not an in-daemon `store/`); Brain reconcile + boot recovery live in `fleet/brain.ts` + `fleet/service.ts` (no separate `recovery/` package); Phase 4 ships `dispatch_fusion` executors for opinion / fusion / plan-fusion plus `fleet/fusion-runs.ts`, `fleet/sessions.ts`, and `prompts/service.ts`. **Phase 5** ships lifecycle auto-validate (`gate-runner` + tool-surface RED proofs / FAIL ledgers / seat fences; `tooling/gates/phase-5.mjs` G1–G9). **Phase 6 first slice** ships Console fusion side-by-side columns, auto-validate evidence UI, Fleet/Analytics log-derived panels, Notifications, and empty/error treatments; **second slice** ships Session Detail + Agent Logs, Run History, session-scoped event projection, and `tooling/gates/phase-6.mjs` G1–G9; **third slice** ships §7.3 quota card sub-rows, Recent Alerts, Model Performance, Settings Billing, type-filtered event replay, and G10 in-app link reachability; **fourth slice** ships ticketed read-only terminal attach on Session Detail (`POST …/attach-ticket` + `WS /v1/pty`, capture-pane poll with per-stream `seq`, G11 single-use ticket proof); **terminal continuity** soak + reconnect live in `phase-6-terminal.mjs` (T1–T2); **fifth slice** ships Network I/O (`/network`, `net.request`, G12), Policies ◆/safety/prompt-diff (G13), and quota archetypes + SSE usage strip (G14; `$AGENTOS_HOME/fake-quota/<provider>.json`). **Phase 8** ships `/afk` FAQ-only auto-answer + `agentos stow`, analytics reconcile ±0 (billing-surface + Brain-token breakdowns), Brain budget handoff (`brain.handoff_triggered` / `brain.handoff_completed`, new session per model), `agentos config doctor`, signed `SelfUpdater` with rollback, soak/canary/fresh-machine/WCAG gates (`tooling/gates/phase-8.mjs` G1–G9), and the Console proof pack under `docs/qa/runs/console-proof-2026-07-25/`. **Phase 9** ships read-only live no-mistakes visibility (`PipelineWatcher` → typed `pipeline.*` events; Console `/pipeline` with transport honesty), `observability.json5` (#11; quiet/working/firehose), and `tooling/gates/phase-9.mjs` G1–G8 + G5b. Task Detail embedded terminal section stays deferred; provider wizard E2E and Figma-fidelity gates ship (`phase-6-wizard.mjs` W1–W4, `phase-6-fidelity.mjs` F1–F4); the 10‑min no-drop-frame soak is measured and gated (`phase-6-terminal.mjs`, 1198 contiguous frames / 600 s; `AGENTOS_TERMINAL_SOAK_MS` for local fast path).
...
| `suggest_cast` | `(roles[]) → BalancerSuggestion` | **[R7] advisory only** — returns what the auto-balancer would spread across the roster (headroom-ranked candidates, basis, refusal when no legal pair exists); the Brain remains the allocator and `resolve_cast` still validates; never sets `familyCheckOverride` |
...
**What changes:** actionable wakes are delivered to the **Brain** — not to a rule engine — as wake digests; the escalation *ladder* becomes the Brain's default playbook (encoded in its editable system prompt + supervision config) rather than hard-coded control flow — **except structural `WEDGED`**, which the substrate detects and ladders itself (respawn once per task+role, then escalate; §11 Phase 3) because a live silent seat must not wait on a judgment call. Deterministic nudge templates remain available to the Brain as a cheap first response (`send_to_crew` with a template ref — still no extra LLM call beyond the Brain's own decision turn).

Wake taxonomy (Rev 2 + [R5]): `PROGRESS`, `TURN_SETTLED`, `STATUS`, `NEEDS_INPUT`, `BLOCKED`, `AUTH_OR_QUOTA`, `BILLING_MISMATCH`, `CONTEXT_PRESSURE`, **`QUOTA_THRESHOLD`** (probe crossed a configured level: window ≥80/95%, low balance, imminent reset — carries the probed metric [R5]), `STALE`, **`WEDGED`** (two structural classes, both enter the same substrate respawn-once-then-escalate ladder bounded by `supervision.respawnPerStage` per task+role): **spin loop** — repeated identical `tool_call` signatures; **stall** — silence past `supervision.staleMinutes.build` while the pane is still live (`lastActivityAt: null` counts as idle since spawn) — deliberately distinct from `SESSION_LOST`, where the pane is gone), `SECURITY`, `DONE`, `FAILED`, `SESSION_LOST` (dead pane — distinct from both WEDGED classes). Default absorb set: `PROGRESS`, mid-stage `TURN_SETTLED`, `CONTEXT_PRESSURE < 70%` — configurable.
...
**Supersession, stated honestly:** R6.2's "no admin shell / marketing-idiom editorial pages" reading is **overridden wherever the Figma file shows dashboard chrome — the Figma file wins** (and it does show an icon rail and top bar throughout the Workspace frames). What survives of R6.2: components live in `packages/ui`, no bespoke lookalikes. *(Historical R6.2 also required marketing render-parity after component promotion; that obligation is **[R9]-void** — there is no marketing app.)* The §7.x ASCII wireframes below (and R6.2's editorial-section descriptions) **demote to information-architecture references** — content and data requirements remain normative; **visual truth lives in Figma**.
...
|---|---|---|---|
| ~~Landing Page · "Orchestrate AI Agents At Scale"~~ | `8:11470` | **[R9] DEAD — no marketing app** | not built |
...
| Dashboard · Task Detail | `37:1845` | `/tasks/[id]` | **Phase 6 first slice live** — fusion columns (`promptsIdentical` + hash), validation evidence (FAIL ≠ GATE_ERROR), Brain decision lane; **10‑min no-drop-frame soak measured** (`phase-6-terminal.mjs`, 1198 contiguous frames / 600 s; requires live ticking pane — gate fails when ticker cannot start); embedded Task Detail terminal section still deferred (read-only attach lives on Session Detail, fourth slice) |
...
| Inference Jobs · Live Log Stream | `41:3973` | `/runs` | **Phase 1 live** (daemon SSE; filters/pause/search/detail); **Phase 9** also surfaces `pipeline.*` frames under the active observability profile |
| (product) · Live no-mistakes pipeline | — | `/pipeline` | **Phase 9 live** — no Figma frame; transport-honest read-only gate view (`wal-assisted` / `interval-only` / `unavailable` + observed lag); evidence `docs/screenshots/pipeline-live.png` + `pipeline-schema-drift.png` |
...
| WS | `/v1/pty?ticket=` | frames: `{type:"pane",seq,target,content}`, `{type:"closed",reason,lastSeq}`, `{type:"notice",reason}` | **[Phase 6 fourth slice + continuity]** redeem ticket once; capture-pane poll (read-only, send-on-change); monotonic per-stream `seq` from 1 (gap = drop; quiet pane sends nothing); every pane frame carries its tmux `target` so reconnect pane identity is observable (not inferred); reconnect = new stream renumbering at 1; `closed.lastSeq` is the last emitted pane seq; client messages → notice, not keystrokes; loopback + exact-origin on upgrade; Console surfaces gaps (`N frames · no gaps` / drop count in red); soak + reconnect gates: `tooling/gates/phase-6-terminal.mjs` |
...
| GET | `/v1/pipeline/status` | → `{ pipeline }` (transport, lagMs, compatibility, active profile knobs) | **[Phase 9]** transport reported as fact (`wal-assisted` / `interval-only` / `unavailable`); never claims `live` until a real push path exists |
| GET | `/v1/pipeline/runs` | → `{ runs, unavailable, reason }` | **[Phase 9]** live SQL-visible set only — empty when unreadable (no event-store ghosts) |
| GET | `/v1/pipeline/log-tails` | → bounded catch-up for active steps | **[Phase 9]** attach-time seed; respects profile `streamPipelineLogs` / `pipelineLogChars`; pure read |

New SSE members **[R3]**: `policy.changed { domain, layer, safetyOverride: boolean }`, `brain.decision { taskId?, tool, rationale }`, `brain.status { state: "running" | "down" | "restarting", queuedWakes: number }`; **[R4 + Phase 8 as-built]**: `brain.handoff_triggered { fromModel, toModel, metric, observedPct, thresholdPct }`, `brain.handoff_completed { fromModel, toModel, fromSessionId, toSessionId, reason }` (legacy `brain.handoff` remains in the schema; emitters use the pair); **[Phase 8]**: `afk.changed`, `afk.auto_answered`; **[R5]**: `quota.updated { connectionId, metrics: QuotaMetric[] }` and `quota.threshold { connectionId, metric, level: "80pct" | "95pct" | "limit-reached" | "low-balance" | "reset-imminent" }`; **[Phase 4]**: `prompt.installed { refs }`, `fusion.side_completed { taskId, runId, role, model, family, promptHash, artifactPath }`, `fusion.completed { taskId, runId, kind, promptsIdentical, aggregatorFamily, contractOk, error? }`; **[Phase 6 fifth slice]**: `net.request { requestId, provider, method, url, status, durationMs, headers (redacted), body… }`; **[Phase 9]**: `pipeline.run_updated` (durable), `pipeline.unavailable` (durable), `pipeline.log_appended` (**live-only** — not written to the event store; no SSE `id:`; attach recovery via `/v1/pipeline/log-tails`). Live SSE is filtered by the active `observability.json5` profile (`surface`); control-plane `daemon.` / `config.` / `policy.` frames always pass. Added to the Rev-2 `OrchestratorEvent` union (which is otherwise retained: `task.state`, `agent.delta/tool/settled/context`, `run.gate`, `provider.*`, `fleet.*`, `fusion.dispatched`, `needs.captain`).
...
│   ├── observability.json5              # [Phase 9 / #11] visibility profiles + pipeline watch
...
- [x] **Claude-SDK gate [R6]:** the subscription-billing path refuses completion until verification passes (G6: `status=409`); isolation defaults written and asserted.
- [x] **Family-classification gate [R6]:** `claude-agent-sdk/<x>` classifies as family `anthropic` in every cross-family check (G8), and the rule is re-derived server-side at both cast and spawn.
...
**Phase 3 — Tool surface, the Brain, fleet execution, SCOUT, watcher, recovery (3 wk)** **[R3 — expanded]** — substrate + real Pi harness path shipped (`tooling/gates/phase-3.mjs` G1–G14; vitest `phase3-*.test.ts`). Console full-live Fleet panels shipped in **Phase 6 first slice** (was deliberately deferred here).
...
- [x] Structural `WEDGED` → respawn once (evidence-stamped) → escalation (G14 + `phase3-wedged.test.ts`). A wedged seat is the nastiest failure mode the fleet has: its **pane is alive**, so every liveness check passes while it produces nothing, holding a worktree lease and a cast slot indefinitely. Detected as silence past `supervision.staleMinutes.build` on a live pane — deliberately distinct from `SESSION_LOST`, where the pane is gone. The ladder is bounded by `supervision.respawnPerStage` per task+role: respawn once, then escalate, because a substrate that respawns forever turns a wedged model into unbounded spend and a second wedge on the same role is far more likely the task than the seat. A seat that never reported anything counts as idle since spawn — `null` must not read as "active now". [A]+[R2]+[R3]
...
**Phase 6 — Console completion (2 wk)** — first–fifth slices shipped (`tooling/gates/phase-6.mjs` **G1–G14**; CI Playwright chromium) plus provider wizard E2E (`phase-6-wizard.mjs` W1–W4), Figma-fidelity (`phase-6-fidelity.mjs` F1–F4), and terminal continuity (`phase-6-terminal.mjs` T1–T2); open items below are embedded-terminal chrome only, not the §7 remainder frame set:
...
- [x] **Task Detail evidence (G6):** Brain decision lane + validation evidence with FAIL vs GATE_ERROR meaning; fusion columns with clean-room proof.
...
- [x] **10-minute terminal no-dropped-frames soak** (`tooling/gates/phase-6-terminal.mjs` T1): measured **1198 contiguous frames over 600 s** (seq 1..1198, zero drops, pane ticker confirmed). **Requires a live ticking pane** (`tmux respawn-pane` must succeed); when the environment cannot fork a ticker, T1 **fails** rather than greening, so a pass always means multi-frame sustained emission was exercised (high frame floor + last frame still arriving near end of window). Default T1 window is 600_000 ms so CI re-proves the duration; set `AGENTOS_TERMINAL_SOAK_MS` shorter for local iteration. Per-column cost/token meters already render on the fusion columns. Session Detail read-only attach shipped in fourth slice above. [A]+[R2]+[R3]
- [ ] Task Detail: embedded terminal section (attach lives on Session Detail today). [A]+[R2]+[R3]
- [x] Provider wizard E2E (`tooling/gates/phase-6-wizard.mjs` W1–W4): a `pi-api-key` connection carries the wizard from doctor through probes driving the real ACTIONS the Console sends (not step names); a fixture `pi-oauth` connection takes the attach-command path; the wizard **resumes at the same step after a daemon restart** with prior verifications intact [R6]; and extra-usage billing is labelled consistently across the provider card, the billing screen and analytics — asserted in both directions, so a screen that *invented* the label when nothing bills that way would also fail. [A]+[R2]
...
- [x] Terminal reconnect resumes the same tmux window with seq continuity (`tooling/gates/phase-6-terminal.mjs` T1–T2). PTY frames now carry a **monotonic per-stream `seq`**: without one, a dropped frame is indistinguishable from a pane that simply did not change, which is exactly the ambiguity "no dropped frames" has to rule out. A quiet pane legitimately sends nothing; a *gap* is a real loss and the Console shows it (`N frames · no gaps`, or the drop count in red). A reconnect is a new stream that restarts its own numbering, so carrying the old sequence over would report a false drop on every reconnect. [B]
...
- [x] **Figma-fidelity gate [R6.3] (`tooling/gates/phase-6-fidelity.mjs` F1–F4):** a manifest maps every Console route to its canonical frame; F1 fails if any served route is neither mapped nor explicitly exempted **with a stated reason** (so a new screen cannot slip in unchecked), F2 requires both halves of the side-by-side on disk, F3 **regenerates the implementation half from the current build at the Figma frame size** — an evidence pack left over from an older build is exactly how a side-by-side stops meaning anything, and F4 requires that additional screens outside the Figma file (permitted where the product needs them) still use the shared shell and design tokens — a prose exemption reason alone is not sufficient. Deliberately a completeness-and-freshness gate rather than a pixel diff: a threshold loose enough to survive font hinting would pass almost any drift. Original text: every shipped screen has a **Figma-frame-vs-implementation side-by-side** in the evidence pack (per breakpoint where the frame specifies); implementation was built from `get_design_context` per screen (figma-design-to-code skill), not eyeballed screenshots; visual diffs reviewed against the canonical frames in the §7 inventory. ~~**Kept from R6.2:** marketing renders identically after component promotion (no-regression parity on marketing routes).~~ **[R9]-void** — no marketing app / routes.
...
- **SQLite+WAL watch with an interval floor; report what is actually attached.** The shipped watcher is a read-only SQLite poll of `state.sqlite`, accelerated by `fs.watch` on the WAL when that file exists, plus per-step log tailing. It reports one of three observed transports as fact: **`wal-assisted`** (WAL watch attached — near-immediate reaction), **`interval-only`** (polling at the configured cadence only), or **`unavailable`** (schema/open failure or watching disabled). A true push **`live`** label is reserved until a real push path exists.
...
Gates:
- [x] Transport honesty: the surface reports the transport actually in use (`wal-assisted` / `interval-only` / `unavailable`) **and the observed lag** (G5 daemon status + G5b Console label). (`live` and the no-mistakes socket `subscribe` path are explicitly not claimed until implemented.)
...
1. *Live visibility into the `no-mistakes` gate* — "when things enter no-mistakes our app has a live view of it rather than just polling". Grounded in an investigation of `no-mistakes v1.40.0`'s actual surfaces rather than assumed capability; the honest finding is that a true push channel exists (socket `subscribe`) but is undocumented and apparently unused even by its own TUI. **Shipped:** read-only SQLite poll + WAL `fs.watch` + step-log tails, reporting `wal-assisted` / `interval-only` / `unavailable` as observed fact. Socket `subscribe` → `live` remains future work (NOT DONE).
