...
- [x] Extension bundle emits `ext.hello` + lifecycle + `ext.usage` over the socket; **persisted AND projected** (`tooling/gates/phase-2b.mjs` P1 — asserts the frames reach the durable log *and* that the same tokens surface in the derived analytics view; a frame no projection reads is not wired up).
...
- [x] Broker locks: login-flow serialisation observed across **processes**, steady-state concurrency, abandoned-lock reclaim (`tooling/gates/phase-7.mjs` G4, where the cross-process broker was built).
