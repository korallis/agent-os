...
| WS | `/v1/pty?ticket=` | frames: `{type:"pane",seq,content}`, `{type:"closed",reason,lastSeq}`, `{type:"notice",reason}` | **[Phase 6 fourth slice + continuity]** redeem ticket once; capture-pane poll (read-only, send-on-change); monotonic per-stream `seq` from 1 (gap = drop; quiet pane sends nothing); reconnect = new stream renumbering at 1; `closed.lastSeq` is the last emitted pane seq; client messages → notice, not keystrokes; loopback + exact-origin on upgrade; Console surfaces gaps (`N frames · no gaps` / drop count in red); soak + reconnect gates: `tooling/gates/phase-6-terminal.mjs` |
