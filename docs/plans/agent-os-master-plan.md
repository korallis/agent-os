...
- [x] **10-minute terminal no-dropped-frames soak** (`tooling/gates/phase-6-terminal.mjs` T1): measured **1198 contiguous frames over 600 s** (seq 1..1198, zero drops, pane ticker confirmed). Default T1 window is 600_000 ms so CI re-proves the duration; set `AGENTOS_TERMINAL_SOAK_MS` shorter for local iteration. Per-column cost/token meters already render on the fusion columns. Session Detail read-only attach shipped in fourth slice above. [A]+[R2]+[R3]
