...
| Dashboard · Token Usage | `37:2265` | `/analytics` | **Phase 6 first slice live** (`GET /v1/analytics`; null/`—` honesty + cost coverage); Model Performance subnav → `/analytics/models` **third slice**; **Phase 8** billing-surface + Brain overhead card with `reconciles ±0`; Network I/O Detail still deferred |
