...
│   │       ├── components/{shell,fleet,runs,sessions,policies,settings,tasks,projects,providers,analytics,notifications,onboarding}/
...
**Phase 6 — Console completion (2 wk)** — first + second slices shipped (`tooling/gates/phase-6.mjs` G1–G9; CI Playwright chromium); remainder open:
- [x] **Browser shell gates (G1, G2, G8):** Console pages render against a seeded daemon (incl. `/notifications`, `/runs/history`); Fleet reflects a new task ≤1 s over SSE; unknown route → shared Not Found. [A]
...
- [ ] Remaining §7 frames: Model Performance, Recent Alerts chrome, Settings Billing, Network I/O Detail.
