...
import { resolveProviderKeyGrant, writeApiKeyFile, ConnectionRegistry, } from "../src/pi/connections.js";
import { buildPiSpawnSpec } from "../src/pi/manager.js";
...
function seedTask(service, options) {
...
describe("spawn env delivery", () => {
    ...
    it("api-key cast grant injects exactly one matching provider key; oauth cast grants none", () => {
        ...
        expect(Object.keys(apiSpec.env).filter((k) => k.endsWith("_API_KEY") || k === "AWS_ACCESS_KEY_ID")).toEqual(["OPENAI_API_KEY"]);
        ...
        expect(Object.keys(oauthSpec.env).filter((k) => k.endsWith("_API_KEY") || k === "AWS_ACCESS_KEY_ID")).toEqual([]);
    });
...
describe("worktree idle reuse moves HEAD to the new branch", () => {
    it("after deliver + re-lease, git HEAD matches the new lease branch", () => {
        ...
        const path = spawn1.data.session.worktreePath;
        const branch1 = spawn1.data.task.branch;
        expect(execFileSync("git", ["-C", path, "rev-parse", "--abbrev-ref", "HEAD"], {
            encoding: "utf8",
        }).trim()).toBe(branch1);
        ...
        const path2 = spawn2.data.session.worktreePath;
        const branch2 = spawn2.data.task.branch;
        expect(path2).toBe(path);
        expect(branch2).not.toBe(branch1);
        expect(execFileSync("git", ["-C", path2, "rev-parse", "--abbrev-ref", "HEAD"], {
            encoding: "utf8",
        }).trim()).toBe(branch2);
    });
});
describe("tool bridge authorization", () => {
    it("refuses orchestration tools from a session that is not the Brain", () => {
        ...
        const refused = service.tools.invokeFromSession("01JCREW0000000000000000000", "deliver_task", { taskId: "01JTASK0000000000000000000" });
        expect(refused.ok).toBe(false);
        expect(refused.error?.code).toBe("UNAUTHORIZED_TOOL");
        const stow = service.tools.invokeFromSession("01JCREW0000000000000000000", "stow_knowledge", {
        ...
        const allowed = service.tools.invokeFromSession("01JCREW0000000000000000000", "read_task", { taskId: "01JTASK0000000000000000000" });
        expect(allowed.error?.code).toBe("NOT_FOUND");
        // Run-tree reads are Brain-only: a crew side must not cross-read peers.
        const artifacts = service.tools.invokeFromSession("01JCREW0000000000000000000", "read_run_artifacts", { taskId: "01JTASK0000000000000000000" });
        expect(artifacts.ok).toBe(false);
        expect(artifacts.error?.code).toBe("UNAUTHORIZED_TOOL");
        const unknown = service.tools.invokeFromSession("01JBR4N0000000000000000000", "definitely_not_a_tool", {});
...
describe("worktree lease reclaim on stop/respawn", () => {
    it("stop/respawn cycles never exceed poolSize leased trees", () => {
        ...
        for (let i = 0; i < poolSize + 2; i++) {
            ...
            const spawn = sessionId === undefined
            ...
            sessionId = spawn.data.session.sessionId;
            const leased = service.worktrees.list().filter((l) => l.state === "leased");
    ...
    it("respawns after dirty stop that quarantined the task branch", () => {
        ...
        const next = respawn.data.session;
        expect(next.worktreePath).not.toBe(session.worktreePath);
        expect(execFileSync("git", ["-C", next.worktreePath, "rev-parse", "--is-inside-work-tree"], {
            encoding: "utf8",
        }).trim()).toBe("true");
        // Prior uncommitted work must still be intact under quarantine.
        expect(readFileSync(join(session.worktreePath, "wip.txt"), "utf8")).toContain("uncommitted builder work");
    });
    it("persists stopped status across restart and does not emit SESSION_LOST", () => {
        ...
        expect(stopped.ok).toBe(true);
        const durable = JSON.parse(readFileSync(join(home, "runs", taskId, "task.json"), "utf8"));
        ...
        expect(restarted.tools.getTask(taskId)?.sessions.find((s) => s.sessionId === sessionId)?.status).toBe("stopped");
...
describe("deliver_task dirty worktree fail-closed", () => {
    it("quarantines and preserves uncommitted files instead of resetting", () => {
        ...
        expect(spawned.ok).toBe(true);
        const path = spawned.data.session.worktreePath;
        ...
        expect(lease?.state).toBe("quarantined");
        expect(readFileSync(join(path, "builder-wip.txt"), "utf8")).toContain("must not be discarded");
        const task = service.tools.invoke("read_task", { taskId });
        expect(task.ok).toBe(true);
        expect(task.data.phase).not.toBe("DONE");
    });
...
describe("delivery invariant choke points", () => {
    it("stamps deliveryBlocked when stop_crewmate quarantines a dirty tree", () => {
        ...
        expect(deliver.ok).toBe(false);
        expect(deliver.error?.code).toBe("CONFLICT");
        expect(service.tools.invoke("read_task", { taskId }).data.phase).not.toBe("DONE");
    });
    it("refuses advance_phase to DONE in favour of deliver_task", () => {
        ...
        expect(service.tools.invoke("read_task", { taskId }).data.phase).toBe("DELIVERING");
    });
    ...
    it("keeps deliveryBlocked sticky across BUILDING→VALIDATING after dirty stop", () => {
        ...
        if (!advanced.ok) {
            ...
            expect(mid.data.deliveryBlocked).not.toBeNull();
        }
        const deliver = service.tools.invoke("deliver_task", { taskId });
        expect(deliver.ok).toBe(false);
        expect(deliver.error?.code).toMatch(/CONFLICT|ILLEGAL_TRANSITION/);
    ...
    it("stamps deliveryBlocked on SCOUT force-quarantine write violation", () => {
        ...
        if (snap.phase === "NEEDS_CAPTAIN") {
            ...
            if (rework.ok) {
                expect(service.tools.invoke("read_task", { taskId }).data.deliveryBlocked).not.toBeNull();
            }
        }
        const deliver = service.tools.invoke("deliver_task", { taskId });
        expect(deliver.ok).toBe(false);
    ...
    it("resolve_delivery_block is Captain-only and keeps dirty tree from DONE", () => {
        ...
        const session = spawned.data
            .session;
        writeFileSync(join(session.worktreePath, "wip.txt"), "still dirty\n");
        expect(service.tools.invoke("stop_crewmate", {
            sessionId: session.sessionId,
            reason: "stop",
        }).ok).toBe(true);
        // Crew / session path cannot clear the block.
        service.tools.setBrainSessionId("01JBR4N0000000000000000000");
        const fromSession = service.tools.invokeFromSession("01JCREW0000000000000000000", "resolve_delivery_block", { taskId, reason: "crew attempt" });
        expect(fromSession.ok).toBe(false);
        expect(fromSession.error?.code).toBe("UNAUTHORIZED_TOOL");
        const fromBrain = service.tools.invokeFromSession("01JBR4N0000000000000000000", "resolve_delivery_block", { taskId, reason: "brain attempt" });
        ...
        expect(resolved.data.deliveryBlocked).toBeNull();
    ...
    it("quarantines when git status cannot prove the tree clean", async () => {
        ...
        try {
            ...
            if (prev === undefined)
                ...
                process.env.AGENTOS_FAKE_GIT = prev;
        }
...
describe("brain spawn failure", () => {
    it("enters BRAIN_DOWN and clears authorized session when Pi spawn fails", () => {
        ...
        const forged = service.tools.invokeFromSession(snap.sessionId, "spawn_crewmate", {
...
describe("extension lifecycle drives session state", () => {
    it("marks a session settled on agent_settled and absorbs PROGRESS", () => {
        ...
        expect(service.tools.listSessions().find((s) => s.sessionId === sessionId)?.status).toBe("running");
        service.handleExtensionFrame({
            type: "ext.lifecycle",
            sessionId,
            phase: "agent_settled",
        ...
        expect(service.tools.listSessions().find((s) => s.sessionId === sessionId)?.status).toBe("settled");
    });
});
describe("crewmate cwd isolation", () => {
    it("never spawns a crewmate cwd equal to the project primary checkout", () => {
        ...
        for (const entry of roles) {
            ...
            if (entry.role === "validator") {
                ...
                const runsDir = artifacts.data.path;
    ...
    it("deliver_task stops a live session before releasing its worktree lease", () => {
        ...
        const deliver = service.tools.invoke("deliver_task", { taskId });
        expect(deliver.ok).toBe(true);
        expect(deliver.data.phase).toBe("DONE");
        expect(service.tools.listSessions().find((s) => s.sessionId === sessionId)?.status).toBe("stopped");
...
describe("terminal task lifecycle choke points", () => {
    it("cancel_task halts live sessions before releasing worktree leases", () => {
        ...
        const sessionId = spawned.data.session.sessionId;
        expect(service.tools.listSessions().find((s) => s.sessionId === sessionId)?.status).toBe("running");
        expect(service.worktrees.list().some((l) => l.taskId === taskId && l.state === "leased")).toBe(true);
        const cancelled = service.tools.invoke("cancel_task", {
            taskId,
            reason: "captain cancelled",
        });
        expect(cancelled.ok).toBe(true);
        expect(cancelled.data.phase).toBe("CANCELLED");
        expect(service.tools.listSessions().find((s) => s.sessionId === sessionId)?.status).toBe("stopped");
        expect(service.worktrees.list().some((l) => l.taskId === taskId && l.state === "leased")).toBe(false);
    ...
    it("refuses spawn_crewmate on terminal tasks for every role", () => {
        ...
        expect(cancelled.ok).toBe(true);
        for (const role of ["builder", "planner", "fusion", "healthcheck", "validator"]) {
            const spawned = service.tools.invoke("spawn_crewmate", {
                taskId,
                role,
                model,
...
describe("gate env hygiene", () => {
    ...
    it("gate subprocess does not inherit a seeded provider key canary", async () => {
        ...
        try {
            runner.writeGateSource(taskId, `const keys = Object.keys(process.env).sort();
...
`, "ts");
            ...
            if (prevCanary === undefined)
                ...
                process.env[canaryKey] = prevCanary;
            if (prevFake === undefined)
                delete process.env.AGENTOS_FAKE_GATE;
            else
                process.env.AGENTOS_FAKE_GATE = prevFake;
        }
    });
});
describe("gate runtime infrastructure is never a verdict", () => {
    it("ts gate timeout discards partial EXPECTED_RED and does not mint RED proof or FAIL ledger", async () => {
        ...
        delete process.env.AGENTOS_FAKE_GATE;
        try {
            runner.writeGateSource(taskId, `console.log("EXPECTED_RED");
...
describe("daemon-owned RED proof (not seat-writable disk)", () => {
    it("ignores forged validation/red-proof.json; only memory+HMAC authorises", async () => {
        ...
        try {
            runner.writeGateSource(taskId, `#!/usr/bin/env python3\nprint("EXPECTED_RED")\nprint("FAIL baseline")\n`);
            ...
            writeFileSync(join(valDir, "red-proof.json"), JSON.stringify({
            ...
            expect(runner.installRedProof(taskId, {
                gateSourceHash: newHash,
                outcome: "EXPECTED_RED",
                provenAt: new Date().toISOString(),
                hmac: "forged",
...
describe("candidate gate isolation and builder worktree ownership", () => {
    it("refuses run_gate(candidate) without an isolated builder worktree", () => {
        ...
        expect(service.tools.invoke("advance_phase", {
            taskId,
            to: "GATE_AUTHORING",
            reason: "test",
        }).ok).toBe(true);
        ...
        try {
            ...
            expect(service.tools.invoke("read_task", { taskId }).data.phase).toBe("GATE_RED_VERIFIED");
    ...
    it("scout spawn does not clobber task.worktreePath used by candidate gates", () => {
        ...
        expect(toBuilding.data.phase).toBe("BUILDING");
        ...
        if (scout.ok) {
            ...
            expect(after.data.worktreePath).toBe(builderPath);
            const scoutPath = scout.data.session
                .worktreePath;
            expect(scoutPath).not.toBe(builderPath);
        }
        else {
            // If scout cannot spawn in BUILDING, still prove builder owns task.worktreePath.
            expect(service.tools.invoke("read_task", { taskId }).data
    ...
    it("stop_crewmate clears task.worktreePath so idle reuse cannot be judged against a stale ref", () => {
        ...
        const makeTask = (title) => {
            ...
            expect(created.ok).toBe(true);
            const taskId = created.data.id;
            expect(service.tools.invoke("resolve_cast", {
                taskId,
                roles: [
                    { role: "builder", model: "openai/gpt-4.1", thinking: "low", cleanRoom: true },
                ],
        ...
        expect(afterStop.data.worktreePath).toBeNull();
        ...
        expect(service.tools.invoke("read_task", { taskId: task1 }).data
            .worktreePath).toBeNull();
        expect(service.tools.invoke("read_task", { taskId: task2 }).data
            .worktreePath).toBe(path2);
    });
