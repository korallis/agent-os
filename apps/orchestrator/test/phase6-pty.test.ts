...
describe("PTY stream lifecycle", () => {
  ...
  let pty: { close: () => Promise<void> } | null = null;
  ...
  async function waitForFrame(
    ...
    throw new Error(
      `timed out waiting for frame; saw ${JSON.stringify(frames)}`,
    );
  }
  ...
  it("streams pane content then closes cleanly when the window is killed", async () => {
    ...
    const pane = await waitForFrame(
      frames,
      (f) => f.type === "pane" && f.content === "hello from pane",
    );
