...
describe("prompt packs", () => {
    ...
    it("renders {{VAR}} and refuses an undefined variable rather than half-rendering", () => {
        ...
        expect(() => service.render("fusion/opinion.md", { QUESTION: "only one" })).toThrow(/CONTEXT/);
    });
    it("lets a project layer override global", () => {
        ...
        expect(service.render("fusion/opinion.md", { QUESTION: "x" }, projectDir).rendered).toBe("project x\n");
    });
...
describe("session keys (G6)", () => {
    it("gives a different directory per model so transcripts never cross families", () => {
        ...
        expect(store.ensure({ projectId: "P", role: "planner", model: "anthropic/claude-fable-5" }).dir).toBe(anthropic.dir);
    ...
    it("wires session dirs into live spawns and hands native Pi session isolation", () => {
        ...
        expect(specA.args[specA.args.indexOf("--session-dir") + 1]).not.toBe(specB.args[specB.args.indexOf("--session-dir") + 1]);
        expect(specA.env.PI_CODING_AGENT_SESSION_DIR).not.toBe(specB.env.PI_CODING_AGENT_SESSION_DIR);
        ...
        expect(withThinking.args[withThinking.args.indexOf("--thinking") + 1]).toBe("high");
        ...
        expect(service.sessionKeys.missingRoles(projectId, [
            { role: "planner", model: "anthropic/claude-fable-5" },
            { role: "planner", model: "openai/gpt-5.6-sol" },
        ...
        expect(service.tools.listSessions().some((s) => s.sessionId === sessionB && s.status === "lost")).toBe(true);
        expect(service.tools.listSessions().some((s) => s.model === "openai/gpt-5.6-sol" &&
            s.sessionId !== sessionB &&
            (s.status === "running" || s.status === "starting"))).toBe(true);
    ...
    it("never inherits ambient session-dir sentinels into crewmate or Brain spawn env", () => {
        ...
        try {
            ...
            const brainModel = brainSnap.model;
            ...
            if (prevAgentos === undefined)
                delete process.env.AGENTOS_SESSION_DIR;
            else
                process.env.AGENTOS_SESSION_DIR = prevAgentos;
            if (prevPi === undefined)
                delete process.env.PI_CODING_AGENT_SESSION_DIR;
            else
                process.env.PI_CODING_AGENT_SESSION_DIR = prevPi;
        }
    });
...
describe("fusion run store", () => {
    it("accumulates per-side telemetry and parses attribution spans", () => {
        ...
        store.recordSideUsage(run.taskId, run.runId, "01JSESSION000000000000000A", { inputTokens: 11, outputTokens: 2, costUsd: 0.1 }, 0);
        ...
        store.writeFused(run.taskId, run.runId, "[ARCHITECT]\nkeep it\n[BUILDER]\nindex it\n[FUSION]\nboth\n");
    ...
    it("discriminates dual-planner side artifacts by index and model", () => {
        ...
        const path0 = store.writeSideArtifact(taskId, runId, 0, "anthropic/claude-fable-5", "side-a\n");
        const path1 = store.writeSideArtifact(taskId, runId, 1, "openai/gpt-5.6-sol", "side-b\n");
...
describe("/opinion live path", () => {
    it("spawns clean-room sides by default, writes artifacts, and completes only after settle", () => {
        ...
        for (const e of sideEvents) {
            if (e.type !== "fusion.side_completed")
                continue;
    ...
    it("honors spawnSides: false bookkeeping opt-out for /opinion", () => {
        ...
        expect(result.ok).toBe(true);
        expect(result.data.spawned).toBe(false);
    ...
    it("forces clean-room argv for /opinion even when cast.cleanRoom is false", () => {
        ...
        const plannerArgvs = capturedArgvs.filter((argv) => argv.some((a) => a.includes("claude-fable-5") || a.includes("gpt-5.6-sol")));
    ...
    it("boot hydrate finalizes mid-dispatch runs left with a never-spawned side", () => {
        ...
        expect(spawnA.ok).toBe(true);
        const sessionA = spawnA.data.session
            .sessionId;
    ...
    it("boot hydrate finalizes sides whose durable sessions are already stopped", () => {
        ...
        const sessionA = spawnA.data.session
            .sessionId;
        const sessionB = spawnB.data.session
            .sessionId;
        ...
        expect(service.fusionRuns.get(taskId, runId)?.completedAt == null).toBe(true);
        expect(service.fusionRuns.get(taskId, runId)?.sides.every((s) => s.settledAt == null)).toBe(true);
        events.length = 0;
        service.tools.hydrateFusionOwnership();
        const run = service.fusionRuns.get(taskId, runId);
        expect(run).not.toBeNull();
    ...
    it("boot hydrate finalizes sides left settled without settledAt", () => {
        ...
        service.tools.hydrateTask({
            ...task,
            sessions: task.sessions.map((s) => s.sessionId === sessionA || s.sessionId === sessionB
                ? { ...s, status: "settled", lastEventAt: now }
                : s),
            updatedAt: now,
        });
        expect(service.tools
            .listSessions()
            .filter((s) => s.sessionId === sessionA || s.sessionId === sessionB)
            .every((s) => s.status === "settled")).toBe(true);
        expect(service.fusionRuns.get(taskId, runId)?.completedAt == null).toBe(true);
        expect(service.fusionRuns.get(taskId, runId)?.sides.every((s) => s.settledAt == null)).toBe(true);
        events.length = 0;
        service.tools.hydrateFusionOwnership();
    ...
    it("does not re-attribute a prior /opinion run's bytes to a sequential same-cast run", () => {
        ...
        for (const side of run2.sides) {
            ...
            expect(detail2.sideArtifacts.some((a) => a.content.includes(side.sessionId))).toBe(true);
        }
    });
    it("finalizes a partial multi-side spawn failure instead of stranding dispatched", () => {
        ...
        const run = runs[0];
        expect(run.sides.every((s) => s.settledAt != null || s.artifactPath !== null)).toBe(true);
        expect(run.completedAt).toBeTruthy();
        ...
        expect(service.tools
            ...
            (s.status === "running" || s.status === "starting"))).toHaveLength(0);
    ...
    it("latches an earlier side failure when a later side settles cleanly", () => {
        ...
        expect(events.filter((e) => e.type === "session.lost" &&
            e.payload.sessionId === sessionA)).toHaveLength(1);
        expect(events.filter((e) => e.type === "session.stopped" &&
            e.payload.sessionId === sessionA)).toHaveLength(0);
    });
    it("deliver_task abort finalizes in-flight fusion sides", () => {
        ...
        const builderPath = spawnBuilder.data.session.worktreePath;
        ...
        const plannerSession = spawnPlanner.data.session.sessionId;
    ...
    it("reconcileMissingCastRoles continues after a failed respawn", () => {
        ...
        expect(events.some((e) => e.type === "captain.escalation" &&
            e.payload.summary.includes("session-key reconcile failed") &&
            e.payload.summary.includes("anthropic/claude-fable-5"))).toBe(true);
        expect(service.tools.listSessions().some((s) => s.model === "openai/gpt-5.6-sol" &&
            s.sessionId !==
                spawnB.data.session.sessionId &&
            (s.status === "running" || s.status === "starting" || s.status === "settled"))).toBe(true);
        // Surviving failure path still marked the wiped first session lost.
        expect(service.tools.listSessions().some((s) => s.sessionId === sessionA && s.status === "lost")).toBe(true);
    });
});
