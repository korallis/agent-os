...
function envelope(seq, ts, event) {
    return {
...
describe("AnalyticsService window + cost honesty", () => {
    it("scopes totals/models/agents to the same day window as daily", () => {
        ...
        const service = new AnalyticsService(() => ({ events: windowEvents, truncated: false }), () => [], () => ({ facts: sessionSpawns, truncated: false }));
    ...
    it("attributes in-window usage to pre-window sessions via side lookup only", () => {
        ...
        const withoutLookup = new AnalyticsService(() => ({ events: windowEvents, truncated: false }), () => [], () => ({ facts: [], truncated: false }));
        expect(withoutLookup.snapshot({ days: 7 }).agents[0]?.role).toBe("unattributed");
        const withLookup = new AnalyticsService(() => ({ events: windowEvents, truncated: false }), () => [], () => ({
            facts: [
                {
                    sessionId,
                    role: "validator",
    ...
    it("surfaces spawn-lookup truncation on the snapshot truncated flag", () => {
        ...
        const service = new AnalyticsService(() => ({ events: windowEvents, truncated: false }), () => [], () => ({ facts: [], truncated: true }));
    ...
    it("tracks partial cost coverage without treating null as zero bill", () => {
        ...
        const service = new AnalyticsService(() => ({ events, truncated: true }), () => []);
    ...
    it("reports absent cost and nullable per-row cost when nothing reported", () => {
        ...
        const service = new AnalyticsService(() => ({ events, truncated: false }), () => []);
