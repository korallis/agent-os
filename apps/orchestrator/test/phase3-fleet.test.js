...
import { canTransition, assertTransition, canRunGate, canSpawnBuilder, canSpawnScout, IllegalTransitionError, } from "../src/substrate/task-machine.js";
const homes = [];
afterEach(async () => {
...
describe("task state machine", () => {
    ...
    it("restores skip-to-BUILDING edges only when red-baseline is off", () => {
        ...
        expect(canTransition("DISPATCH_RESOLVED", "BUILDING", { redBaselineRequired: false })).toBe(true);
...
describe("phase 3 fleet integration", () => {
    it("create_task, resolve_cast policy, scripted local-only SHIP", async () => {
        ...
        try {
            ...
            expect((await cast.json()).ok).toBe(true);
    ...
    it("BRAIN_DOWN queues wakes and blocks orchestration tools", async () => {
        ...
        writeFileSync(join(home, "config", "brain.json5"), `{ cast: "auto", thinking: "high", preferenceOrder: ["best-available via any api-key"], handoff: { thresholdPct: 80, target: "same-family-api-key" }, respawnBlocked: true }\n`);
        process.env.AGENTOS_FAKE_TMUX = "1";
        process.env.AGENTOS_FAKE_PI = "1";
        process.env.AGENTOS_FAKE_BRAIN = "1";
        const daemon = await startDaemon({ home, port: 0, stdout: false });
