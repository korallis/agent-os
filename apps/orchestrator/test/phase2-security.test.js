...
import { assertNoAmbientAnthropicKey, EnvHygieneError, scrubEnv, } from "../src/security/env-scrub.js";
import { containsSecretMaterial, scanForSecrets, SECRET_CANARY, scrubSecrets, } from "../src/security/secret-canary.js";
...
import { assertCrossFamilyCast, CrossFamilyViolationError, isClaudeAgentSdkModel, } from "@agentos/claude-agent-sdk-pi";
describe("env hygiene (§4.8)", () => {
    ...
    it("flags ambient ANTHROPIC_API_KEY for subscription-sdk path", () => {
        expect(() => assertNoAmbientAnthropicKey({ ANTHROPIC_API_KEY: "sk-ant-ambient" })).toThrow(EnvHygieneError);
...
describe("family classification [R6]", () => {
    it("classifies claude-agent-sdk/* as anthropic", () => {
        ...
        expect(familiesConflict("claude-agent-sdk/claude-opus-4-5", "anthropic/claude-fable-5")).toBe(true);
        expect(() => assertCrossFamilyCast("claude-agent-sdk/claude-opus-4-5", "anthropic/claude-sonnet-4-5")).toThrow(CrossFamilyViolationError);
        expect(() => assertCrossFamilyCast("claude-agent-sdk/claude-opus-4-5", "openai/gpt-5.6-codex")).not.toThrow();
    });
});
