...
import type { ProviderConnection, QuotaConfig } from "@agent-os/protocol";
...
function conn(partial: Partial<ProviderConnection> & Pick<ProviderConnection, "provider">): ProviderConnection {
...
describe("quota probes", () => {
  it("parses Claude live metrics from fixture response", async () => {
    ...
    expect(sample.metrics.some((m) => m.tier === "live" && m.kind === "session-window-pct")).toBe(
      true,
    );
  ...
  it("LIMIT REACHED fixture emits threshold event", async () => {
    const { sample, events } = await probeConnection({
      ...
      fixtureToken: "fixture-token",
      fetchImpl: async () =>
        new Response(JSON.stringify({ used_percent: 100 }), { status: 200 }),
    });
