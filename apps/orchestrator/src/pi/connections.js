...
import { providerConnectionSchema, } from "@agent-os/protocol";
...
export class ConnectionRegistry {
    ...
    load() {
        ...
        try {
            ...
            for (const item of raw) {
                ...
                if (parsed.success)
                    this.connections.set(parsed.data.id, parsed.data);
            }
        }
        catch {
    ...
    persist() {
        ...
        writeFileSync(this.storePath(), JSON.stringify([...this.connections.values()], null, 2), { mode: 0o600 });
    }
    list() {
    ...
    findByProviderKind(provider, kind) {
        return ([...this.connections.values()].find((c) => c.provider === provider && c.kind === kind) ?? null);
    }
    /**
    ...
    syncFromAuthStore() {
        ...
        for (const p of presence) {
            ...
            const existing = [...this.connections.values()].find((c) => c.provider === p.provider && c.kind === "pi-oauth");
            if (existing !== undefined) {
                ...
                if (presenceUnchanged(existing.authStorePresence, p) &&
        ...
        for (const existing of this.connections.values()) {
            if (existing.billingMode !== "subscription-sdk")
                continue;
    ...
    upsertConnection(input) {
        ...
        if (billingMode === "subscription-sdk")
            billingSurface = "sdk-credit-pool";
        if (billingMode === "extra-usage-oauth")
            billingSurface = "extra-usage-per-token";
        if (billingMode === "api-key")
    ...
    setLimitReached(id, reached, reason) {
    ...
    buildConnection(input) {
        ...
        let billingSurface = meta.defaultBilling;
        if (input.billingMode === "subscription-sdk")
            billingSurface = "sdk-credit-pool";
        if (input.billingMode === "extra-usage-oauth")
            billingSurface = "extra-usage-per-token";
        if (input.billingMode === "api-key")
            billingSurface = "api-metered";
        const billingMode = input.billingMode ?? null;
        const now = new Date().toISOString();
        const health = billingMode === "subscription-sdk"
...
function presenceUnchanged(existing, next) {
    if (existing === null)
        return false;
...
function resolveCredentialPath(kind, billingMode) {
    if (billingMode === "subscription-sdk")
...
export function writeApiKeyFile(home, provider, apiKey) {
...
export function readApiKeyFile(home, provider) {
 ...
 */
export function resolveProviderKeyGrant(agentosHome, model, connections) {
