...
import { decideHandoff, worstWindowPctFromSample, } from "./brain-handoff.js";
/**
 ...
 */
const DEFAULT_BRAIN_MODEL_BY_PROVIDER = {
...
function modelRoutingProvider(model) {
...
export class FleetService {
     ...
     */
    seedHandoffCooldownFromDurable() {
    ...
    summary() {
        ...
        const active = tasks.filter((t) => ["BUILDING", "VALIDATING", "PLANNING", "GATE_AUTHORING", "DELIVERING", "PLAN_FUSED", "GATE_RED_VERIFIED", "DISPATCH_RESOLVED"].includes(t.phase)).length;
        ...
        const doneToday = tasks.filter((t) => t.phase === "DONE" && t.updatedAt.startsWith(today)).length;
    ...
    reconcileBrainPane() {
        ...
        if (snap.status !== "running" && snap.status !== "starting")
            return;
        if (snap.tmuxWindow === null)
            return;
        if (this.tmux.hasWindow(snap.tmuxWindow))
            return;
     ...
     */
    evaluateBrainHandoff() {
        const brainSnapshot = this.brain.getSnapshot();
        if (brainSnapshot.model === null)
            return null;
        const cfg = this.options.config.effective().config;
        ...
        const brainConnection = connections.find((c) => c.provider.toLowerCase() === brainProvider) ?? null;
     ...
     */
    handoffCandidates(currentModel) {
        const connections = this.options.connections?.list() ?? [];
        const samples = this.options.quotaSamples?.() ?? [];
        const currentProvider = modelRoutingProvider(currentModel);
        const candidates = [];
        for (const connection of connections) {
            if (connection.limitReached)
                continue;
            if (connection.health === "unhealthy")
                continue;
            if (connection.provider.toLowerCase() === currentProvider)
                continue;
...
function taskToListItem(task, projectName) {
