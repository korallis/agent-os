...
export class SessionKeyStore {
  ...
  static computeKey(input: { projectId: string; role: string; model: string }): string {
   ...
   */
  ensure(input: { projectId: string; role: string; model: string }): SessionKey {
    const key = SessionKeyStore.computeKey(input);
    const dir = join(this.root, key);
    const metaPath = join(dir, "session.json");
    if (existsSync(metaPath)) {
      try {
        return JSON.parse(readFileSync(metaPath, "utf8")) as SessionKey;
      } catch {
  ...
  missingRoles(
    projectId: string,
    expected: Array<{ role: string; model: string }>,
  ): Array<{ role: string; model: string }> {
    return expected.filter(
      ({ role, model }) =>
        !existsSync(join(this.root, SessionKeyStore.computeKey({ projectId, role, model }))),
    );
  }
}
