...
export class BrainManager {
    ...
    start(reason = "boot") {
        ...
        if (this.fake) {
            ...
            if (this.deps.pi?.binary == null ||
                ...
                this.enterDown(this.deps.pi?.binary == null
            ...
            this.deps.tools.setBrainSessionId(sessionId);
            try {
                const grant = resolveProviderKeyGrant(this.deps.home, model, this.deps.connections);
    ...
    loadHandoff() {
        if (!existsSync(this.handoffStatePath))
            return null;
        try {
            const raw = JSON.parse(readFileSync(this.handoffStatePath, "utf8"));
            if (typeof raw !== "object" ||
                raw === null ||
                typeof raw.toModel !== "string" ||
                raw.toModel.length === 0) {
                return null;
            }
            const record = raw;
            return {
    ...
    teardownPriorBrain() {
        ...
        const targets = new Set(["agentos:brain"]);
        if (priorWindow !== null)
            targets.add(priorWindow);
     ...
     */
    runScriptedLocalShip(taskId, model = "openai/gpt-4.1") {
        ...
        if (spawned.ok === true && spawned.data !== undefined) {
            const sessionId = spawned.data.session
                ?.sessionId;
    ...
    resolveModel() {
        ...
        const healthy = connections.filter((c) => c.health === "healthy" || c.health === "unknown" || c.health === "degraded");
        for (const pref of this.config.preferenceOrder) {
            if (pref.includes("anthropic") && pref.includes("claude")) {
                const c = healthy.find((x) => x.provider === "anthropic" || x.provider === "claude-agent-sdk");
                if (c !== undefined) {
            ...
            if (pref.includes("anthropic") && pref.includes("api-key")) {
                const c = healthy.find((x) => (x.provider === "anthropic" ||
