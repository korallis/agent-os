...
import { z } from "zod";
import { apiKeyConnectRequestSchema, brainToolNameSchema, afkRequestSchema, configDomainSchema, configLayerSchema, oauthStartRequestSchema, onboardingAdvanceRequestSchema, PI_PINNED_VERSION, projectRegisterRequestSchema, SAFETY_CONFIRM_HEADER, safetyPoliciesConfigSchema, taskCreateRequestSchema, toolCallRequestSchema, } from "@agent-os/protocol";
import { ConfigWriteError } from "../config/service.js";
...
function sendError(reply, status, code, message, issues = null) {
...
export function buildServer(deps) {
    ...
    app.addContentTypeParser(["application/json5", "text/plain"], { parseAs: "string" }, (_req, body, done) => done(null, body));
    ...
    function parseScopedEventsQuery(raw) {
        const query = scopedEventsQuerySchema.safeParse(raw);
        if (!query.success) {
            return { ok: false, message: "invalid events query" };
        }
        const types = query.data.types === undefined || query.data.types.trim() === ""
            ? null
            : query.data.types
                .split(",")
                .map((t) => t.trim())
     ...
     */
    app.get("/v1/tasks/:id/events", (request, reply) => {
        const parsed = parseScopedEventsQuery(request.query);
        if (!parsed.ok) {
            sendError(reply, 400, "BAD_REQUEST", parsed.message);
            return undefined;
        }
        const { events, truncated } = deps.store.eventsForTask(request.params.id, parsed.types, parsed.limit);
        return { taskId: request.params.id, events, truncated };
    });
    // ── Sessions: Agent Detail (41:2) + Agent Logs (41:456) ─────────────────
    app.get("/v1/sessions/:id", (request, reply) => {
        if (deps.fleet === undefined) {
            sendError(reply, 404, "NOT_FOUND", "fleet service unavailable");
            return undefined;
        }
        const session = deps.fleet.tools.listSessions().find((s) => s.sessionId === request.params.id) ?? null;
        if (session === null) {
    ...
    app.get("/v1/sessions/:id/events", (request, reply) => {
        const parsed = parseScopedEventsQuery(request.query);
        if (!parsed.ok) {
            sendError(reply, 400, "BAD_REQUEST", parsed.message);
            return undefined;
        }
        const { events, truncated } = deps.store.eventsForSession(request.params.id, parsed.types, parsed.limit);
     ...
     */
    app.get("/v1/runs/history", (_request, reply) => {
        if (deps.fleet === undefined) {
            sendError(reply, 404, "NOT_FOUND", "fleet service unavailable");
            return undefined;
        }
    ...
    app.post("/v1/sessions/:id/attach-ticket", (request, reply) => {
    ...
    app.get("/v1/tasks/:id/fusion", async (request, reply) => {
        if (deps.fleet === undefined) {
            sendError(reply, 404, "NOT_FOUND", "fleet service unavailable");
            return;
        }
    ...
    app.get("/v1/tasks/:id/fusion/:runId", async (request, reply) => {
        if (deps.fleet === undefined) {
            sendError(reply, 404, "NOT_FOUND", "fleet service unavailable");
            return;
        }
    ...
    app.get("/v1/prompts/diff", async (request, reply) => {
        if (deps.prompts === undefined) {
            sendError(reply, 404, "NOT_FOUND", "prompt service unavailable");
            return;
        }
        ...
        try {
            ...
            sendError(reply, 400, "BAD_REQUEST", error instanceof Error ? error.message : "invalid prompt ref");
            return;
        }
    });
     ...
     */
    app.get("/v1/network", async (request, reply) => {
        ...
        void reply;
        return {
            requests: events.flatMap((envelope) => envelope.event.type === "net.request"
                ? [{ ...envelope.event.payload, ts: envelope.ts }]
                : []),
            truncated,
        };
    });
    app.get("/v1/network/:id", async (request, reply) => {
        const { events } = deps.store.eventsOfTypes(["net.request"], 1000);
        for (const envelope of events) {
            if (envelope.event.type !== "net.request")
                continue;
            if (envelope.event.payload.requestId !== request.params.id)
                continue;
    ...
    app.post("/v1/tools/call", async (request, reply) => {
        ...
        const result = body.data.sessionId !== undefined
            ? deps.fleet.tools.invokeFromSession(body.data.sessionId, toolParsed.data, body.data.input)
            : deps.fleet.tools.invoke(toolParsed.data, body.data.input, body.data.idempotencyKey !== undefined
                ? { idempotencyKey: body.data.idempotencyKey }
                : {});
        return {
    ...
    app.get("/v1/connections/:id/quota", async (request, reply) => {
    ...
    app.post("/v1/connections/:id/quota/refresh", async (request, reply) => {
        if (deps.quotaSamples === undefined ||
        ...
        const authJsonPaths = resolveAuthJsonPathsWithFallback(deps.pi.isolationMode === "managed" ? deps.pi.managedHome : null);
            ...
            switch (body.data.action) {
                ...
                case "enable-probes": {
                    ...
                    if (result === "empty-selection") {
                        sendError(reply, 400, "BAD_REQUEST", "select and verify at least one provider before enabling probes");
                        return;
                    }
    ...
    app.get("/v1/events/replay", (request, reply) => {
        const query = replayQuerySchema.safeParse(request.query);
        if (!query.success) {
            sendError(reply, 400, "BAD_REQUEST", "invalid replay query");
            return undefined;
        }
        const typeFilter = query.data.types !== undefined && query.data.types.length > 0
            ? query.data.types
                .split(",")
                .map((t) => t.trim())
                .filter((t) => t.length > 0)
        ...
        if (query.data.order === "desc") {
            const { events, truncated } = deps.store.eventsBeforeId(query.data.before ?? null, query.data.limit);
            return { events, truncated };
        }
        const { events, truncated } = deps.store.eventsAfterId(query.data.after ?? null, query.data.limit);
        return { events, truncated };
    });
    /**
    ...
    app.get("/v1/events", (request, reply) => {
        ...
        const cursor = typeof headerCursor === "string" && headerCursor.length > 0
            ? headerCursor
        ...
        const enqueueWrite = (chunk) => {
            const result = writeChain.then(() => safeWrite(chunk), () => safeWrite(chunk));
            writeChain = result.then(() => undefined, () => undefined);
            return result;
        };
        const writeFrame = (envelope) => enqueueWrite(`id: ${envelope.id}\nevent: ${envelope.event.type}\ndata: ${JSON.stringify(envelope)}\n\n`);
        void (async () => {
            let live = false;
            try {
                ...
                for (;;) {
                    ...
                    for (const envelope of page.events) {
                        ...
                        if (!(await writeFrame(envelope)))
                            return;
                    }
                    if (!page.truncated || page.events.length === 0)
                        break;
    ...
    app.get("/v1/config/:layer/:domain", (request, reply) => {
        ...
        if (layer === "project" || layer === "task") {
            sendError(reply, 400, "LAYER_NOT_WRITABLE", `${layer} layers are trust-gated/task-scoped and land in Phase 2+`);
            return undefined;
        }
    ...
    app.put("/v1/config/:layer/:domain", (request, reply) => {
        ...
        const { layer, domain } = params.data;
        if (layer !== "global") {
            sendError(reply, 400, "LAYER_NOT_WRITABLE", layer === "shipped"
                ? "shipped defaults are read-only by design (§2.6)"
                : `${layer} layers are trust-gated/task-scoped and land in Phase 2+`);
            return undefined;
        }
        const body = request.body;
        const json5Text = typeof body === "string" ? body : JSON.stringify(body ?? {}, null, 2);
        ...
        if (domain === "policies") {
            ...
            if (confirm !== "true" && confirm !== "yes") {
                sendError(reply, 428, "CONFIRMATION_REQUIRED", `safety-policy writes require the ${SAFETY_CONFIRM_HEADER}: true header (Captain-only, §2.6 #12)`);
                return undefined;
            }
        }
        try {
            ...
            return { applied: true, domain, layer, contentHash };
        }
        catch (error) {
            if (error instanceof ConfigWriteError) {
                sendError(reply, 400, "CONFIG_INVALID", `invalid ${error.layer} config for domain "${error.domain}" — nothing applied`, error.issues);
                return undefined;
            }
            throw error;
        }
 ...
 */
function enableProbesForOnboarding(deps, svc) {
    ...
    if (selected.length > 0) {
        ...
        enableQuotaProviders(deps.config, toEnable.map((p) => ({
            provider: p.provider,
    ...
    if (presence.length === 0)
        return "empty-selection";
    enableQuotaProviders(deps.config, presence.map((p) => ({ provider: p.provider, billingMode: null })));
    return "ok";
}
/**
 ...
 */
function syncAnthropicBillingFromWizard(deps, state) {
    if (deps.connections === undefined)
        return;
    const claude = state.providers.find((p) => p.provider === "anthropic" && p.selected && p.claudeBillingMode !== null);
    if (claude === undefined || claude.claudeBillingMode === null)
        return;
...
function buildOAuthAttachCommand(options) {
    ...
    const useManaged = options.pi === undefined ||
