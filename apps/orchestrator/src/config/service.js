...
import { CONFIG_DOMAINS, configDomainSchemas, } from "@agent-os/protocol";
import { issuesFromZodError, loadLayerFile, resolveAll, resolveDomain, sha256, } from "./resolver.js";
export class ConfigWriteError extends Error {
    domain;
    layer;
    issues;
    constructor(domain, layer, issues) {
...
export class ConfigService {
     ...
     */
    reload(hotReloaded) {
        const previousByDomain = new Map(CONFIG_DOMAINS.map((domain) => [domain, JSON.stringify(this.resolved.config[domain])]));
        const globalOverrides = {};
        const retainedRejections = [];
        for (const domain of CONFIG_DOMAINS) {
            try {
                const file = loadLayerFile(this.globalDir, domain);
                if (file === null) {
        ...
        for (const domain of CONFIG_DOMAINS) {
            if (JSON.stringify(this.resolved.config[domain]) === previousByDomain.get(domain))
                continue;
            const contentHash = this.lastGoodGlobal.get(domain)?.contentHash ??
    ...
    writeGlobal(domain, json5Text) {
        ...
        try {
            ...
            throw new ConfigWriteError(domain, "global", [
                { path: "", message: `JSON5 parse error: ${error.message}` },
            ]);
        }
        const shipped = loadLayerFile(this.shippedDir, domain);
        if (shipped === null)
            throw new Error(`missing shipped default for "${domain}"`);
    ...
    handleExternalChange() {
        ...
        for (const domain of CONFIG_DOMAINS) {
            const probe = probes.get(domain);
            if (probe === undefined || probe.kind === "missing")
                continue;
            ...
            const { file } = probe;
            if (this.appliedHashes.get(domain) === file.contentHash)
                continue;
            const issues = this.validateGlobal(domain, file.value);
            this.appliedHashes.set(domain, file.contentHash);
            if (issues.length > 0) {
