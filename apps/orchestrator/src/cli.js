import { healthResponseSchema, statusResponseSchema, } from "@agent-os/protocol";
import { startDaemon } from "./daemon.js";
import { formatConfigDoctor } from "./prompts/doctor.js";
...
export async function runStatus() {
    ...
    console.log(`agentosd: RUNNING  v${parsedHealth.data.version}  pid ${parsedHealth.data.pid}  ${base}`);
    try {
...
export async function runAfk(mode, duration) {
    const base = `http://${LOOPBACK_HOST}:${cliPort()}`;
    let token;
    try {
 ...
 */
export async function runStow(projectId, notes) {
...
export async function runConfigDoctorCommand() {
    ...
    try {
        ...
        const body = (await response.json());
        for (const line of formatConfigDoctor(body.doctor))
            console.log(line);
    }
    catch {
