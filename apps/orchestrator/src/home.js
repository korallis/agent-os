...
export class HomeLockError extends Error {
    ...
    holderPid;
    code = "HOME_LOCKED";
    constructor(message, home, lockPath, holderPid) {
        super(message);
        this.home = home;
...
export function acquireHomeLock(home) {
    ...
    if (fd === null) {
        ...
        let holderPid = readLockPid(lockPath);
        while (holderPid === null && Date.now() < deadline) {
            sleepMs(LOCK_PID_POLL_MS);
            holderPid = readLockPid(lockPath);
        }
        if (holderPid !== null && isProcessAlive(holderPid)) {
            throw new HomeLockError(`AGENTOS_HOME already in use by agentosd pid ${holderPid} (lock: ${lockPath})`, home, lockPath, holderPid);
        }
        ...
        if (fd === null) {
            ...
            throw new HomeLockError(`AGENTOS_HOME already in use${retryPid !== null ? ` by agentosd pid ${retryPid}` : ""} (lock: ${lockPath})`, home, lockPath, retryPid);
        }
    }
    try {
 ...
 */
export function ensureDaemonToken(home) {
