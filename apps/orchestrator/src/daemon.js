...
import { acquireHomeLock, ensureDaemonToken, ensureHome, resolveHome, } from "./home.js";
...
import { hydrateQuotaSamples, QuotaProbeScheduler, } from "./quota-probes/scheduler.js";
...
export const EXPECTED_EXTENSION_DIST = join(here, "..", "..", "..", "packages", "pi-extension", "dist", "extension.js");
/**
...
export async function startDaemon(options = {}) {
    ...
    const lock = acquireHomeLock(home);
    ...
    try {
        ...
        const streams = [{ level: "info", stream: fileDestination }];
        ...
        const quotaSamples = hydrateQuotaSamples(store);
        ...
        if (extensionPath === undefined) {
            logger.warn({ expected: EXPECTED_EXTENSION_DIST }, "agent-os Pi extension dist not found — Brain and crewmate spawn will fail closed (PI_UNAVAILABLE / BRAIN_DOWN); build packages/pi-extension first");
        }
        ...
        const analytics = new AnalyticsService(({ days, limit }) => {
            ...
            const { events: spawns, truncated: spawnTruncated } = eventStore.eventsByType("session.spawned", 100_000);
            const facts = [];
        ...
        const pty = attachPtyServer({
            ...
            tickets: ptyTickets,
            tmux: fleet.tmux,
            resolveTarget: (sessionId) => fleet.tools.listSessions().find((s) => s.sessionId === sessionId)?.tmuxWindow ?? null,
        });
        ...
        return {
            ...
            logger,
            async close(reason = "shutdown", signal) {
                if (closed)
                    return;
                closed = true;
                try {
                    ...
                    await new Promise((resolve) => setTimeout(resolve, SSE_SHUTDOWN_GRACE_MS));
